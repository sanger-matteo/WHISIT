function cell_Track = Convert_Clone2Track(clone_List)
%
...
cell_Track = cell( 1 , length(uniqueTracks) );
for ii = 1 : length( uniqueTracks )
    ...
    cell_Track{1, ii} = [ clone_List{ idx_trk(idx_birth) } ] ;     
    if length(idx_trk) == 1
        cell_Track{2, ii} = cell2mat( TrackInfo( 2, idx_trk(idx_birth(1)) )) ;       % first tracked frame
        cell_Track{3, ii} = [] ;                                                     % divisions
        cell_Track{4, ii} = cell2mat( TrackInfo( 3, idx_trk(idx_birth(1)) )) ;       % last tracked frame
        
    elseif length(idx_trk) > 1
        cell_Track{2, ii} = cell2mat( TrackInfo( 2, idx_trk(idx_birth(1)) )) ;       % first tracked frame
        cell_Track{3, ii} = cell2mat( TrackInfo( 2, idx_trk(idx_birth(2:end)) )) ;   % divisions
        cell_Track{4, ii} = cell2mat( TrackInfo( 3, idx_trk(idx_birth(end)) )) ;     % last tracked frame

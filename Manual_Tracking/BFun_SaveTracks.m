...
% --- STEP X --- Order CellTrack ------------------------------------------
% When tracking the user delete and add cell tracks at different times. The
% order of track in the cTrack can therefore be quite "random". To avoid
% error and crashes, function Lng_Create_clone_List absolutely need that
% any mother cell is positioned before daughter cell(s) (in respect to the
% column order in cTrack)
% Here we ensure that by reordering them all according to birth frame: 

reord = [];
for cc = 1 : size(CellTracks, 2) 
    reord(1,cc) =  CellTracks{1, cc};
    reord(2,cc) =  min(find((CellTracks{2, cc}(:,1) ~= 0)));
end
[l_sort, idx] = sort(reord(2,:));
CellTracks = CellTracks(:, idx);

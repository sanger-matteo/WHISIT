...
%
%Combine_Det2Track - The function use the manual tracking data (track.txt)
%  and combine it with the detection done in Oufti. 
%  The tracking performed using WHISIT is used to reorder the cellList to
%  create a new time tracked cellsList. The algorithm also track the
%  polarity of the cell, from each new cell division onward, in order to
%  keep track which is the new and which is the old pole
%
%
% ----- INPUT -------------------------------------------------------------
    ...
    for ff = t_range(1) : t_range(end)                % <<<----- CATCH_1 (at line ~370)
      if ~isempty( old_cList{ff}{M_coID(ff,cc)})  &&  size( old_cList{ff}{M_coID(ff,cc)}.mesh, 2) == 4
        ...
        if isempty( cTrack{5, cc}(cTrack{5, cc}(:)~=0 ) )          % it is a founder cell   
            ...
            new_cList{ff}{pcd}.frame_birth = t_range(1) ;             % take first frame tracked
        else
            new_cList{ff}{pcd}.track_ancestor = cTrack{5, cc}(cTrack{5, cc}(:)~=0 ) ;  
            new_cList{ff}{pcd}.frame_birth = t_range(2) ;             % take the second frame tracked
        end
        
        new_cList{ff}{pcd}.tracks_offspring  = cTrack{6, cc}(cTrack{6, cc}(:)~=0 ) ;                           
        new_cList{ff}{pcd}.frame_nextgen = find(cTrack{6, cc}(:)~=0) ;
        new_cList{ff}{pcd}.frame_last           = t_range(end) ;
...
for cc = 1 : size(cTrack, 2)                    % go through each cell-track  
  ...
  if ~isempty(t_range)                          % if cell-track is empty, ignores it 
    ...
    pcd = pcd_trk{2,cc}(t_range(1));                                        % find the position of cc-th cell-track in new_cList
    tid_Anc = new_cList{1,t_range(1)}{pcd}.track_ancestor ;                 % find the track ID of Ancestor      
    Fr_Brt = new_cList{1,t_range(1)}{pcd}.frame_birth ;                        % N.B.: we put +1 in next for-loop f_ii

function [out1,out2,out3] = grabinput(arg1)
%GRABINPUT = A modified version of standard grabinput function in Matlab.
%   Like the original it takes graphical input from mouse. In addition the
%   visual curso have been changed to a red crosshair.
%
% [X,Y] = GRABINPUT(N) gets N points from the current axes and returns
...
%
% [X,Y] = GRABINPUT gathers an unlimited number of points until the
%   return key is pressed.
%
% [X,Y,BUTTON] = GRABINPUT(N) returns a third result, BUTTON, that
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    % Check Inputs
    if nargin == 0
        how_many = -1;      % how_many stores the number of input arguments
        b = [];
    else
        how_many = arg1;
        b = [];
    ...
    fig = gcf;               % Get latest displayed figure ...
    figure(APP_opt.fig);     % and make it the currently handled figure
    gca(fig);                % Make sure the figure has an axes    
    % Setup the figure to disable interactive modes and activate pointers. 
    initialState = setupFcn(fig);
    
    % onCleanup in-built function creates an object that, when destroyed, 
    % executes the function cleanupFun and restore everything to original 
    % state in event of completion, closing of figure errors or ctrl+c. 
    c = onCleanup(@() restoreFcn(initialState));
    
    drawnow          % Update figures and process callbacks
    char = 0;
    waserr = 0;     % was_error check correct "click button occured" 
                    % (like if figure was closed prematurely)
        
    while how_many ~= 0 && waserr ~= 1
        waserr = 0;
        try
            keydown = wfbp;
        catch %#ok<CTCH>
        ...
        if(waserr == 1)
            if(ishghandle(fig))     % check if graphics object handles exsists
                cleanup(c);
                error(message('MATLAB:ginput:Interrupted'));
            else
                cleanup(c);
                % Removed error callback: it prevents correct clean up if 
                % figure is manually closed by the user
                % error(message('MATLAB:ginput:FigureDeletionPause'));
            end
        end
        
        if(waserr ~= 1)         % if image was not closed manually by user
...
set(crossHair(1), 'Position', [cp(1)-gap        cp(2)             gap thickness]);
set(crossHair(2), 'Position', [cp(1)+thickness  cp(2)             gap thickness]);
set(crossHair(3), 'Position', [cp(1)            cp(2)-gap         thickness gap]);
set(crossHair(4), 'Position', [cp(1)            cp(2)+thickness   thickness gap]);
...
for k = 1:4
    crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', [0.5 0 0], 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
end
end

function [long_axis, short_axis, X0_in, Y0_in, orientation_rad] = Geom_My_fit_ellipse__v2( x , y )
%
...
% parametric vector: extract parameters from the conic equation
[a,b,c,d,e] = deal( a(1),a(2),a(3),a(4),a(5) );

% remove the orientation from the ellipse
if ( min(abs(b/a),abs(b/c)) > orientation_tolerance )
    if a > c         % if a > c the oriantation_rad calculation gives
        temp = a;    % unccorect negative values
        a = c;
        c = temp;
    end    
...
if (test>0)
    ...
    X0_in       = P_in(1);
    Y0_in       = P_in(2);
    epsilon     = ( (long_axis^2 - short_axis^2) / short_axis^2 );      % ellipse eccentricity

...
[IC_V , Rel_V, Distr_Vals] = Eval_RawValue_Extrema(track_List);
V_Min = Rel_V(1);     % 1 --> min;
V_Max = Rel_V(2);     % 2 --> max;
...
for cc = 1 : length(track_List)    
    ...
    for ff = 1 : length(track_List{cc})        
        ...
        switch APP_opt.t3_PlotOpt_C
            case 1      % Whole Cell
                Value = mean(track_List{cc}{ff}.CH1.IC(  track_List{cc}{ff}.Mask_wCell )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range );
            case 2      % Membrane
                Value = mean(track_List{cc}{ff}.CH1.IC(  track_List{cc}{ff}.Mask_Memb )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
            case 3      % Old_PL                   
                Value = mean(track_List{cc}{ff}.CH1.IC(  track_List{cc}{ff}.CH1.Mask_PL_1 )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
            case 4      % New_PL                    
                Value = mean(track_List{cc}{ff}.CH1.IC(  track_List{cc}{ff}.CH1.Mask_PL_2 )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
        end
...
for cc = 1 : length(track_List)
    for ff = 1 : length(track_List{cc})
        ...
        if I_max < max( max(track_List{cc}{ff}.CH1.IC))
            I_max = max( max(track_List{cc}{ff}.CH1.IC) );
        end
        if I_min > min( min(track_List{cc}{ff}.CH1.IC))
            I_min = min( min(track_List{cc}{ff}.CH1.IC));
        end 

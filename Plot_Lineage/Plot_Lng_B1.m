...
for cc = 1 : length(cN_EndLine)
    ...
    for ff = 1 : length(clone_List{idx_EndL})               
        ...
        switch APP_opt.t3_PlotOpt_C
            case 1      % Whole Cell
                Value = mean(clone_List{idx_EndL}{ff}.CH1.IC(  clone_List{idx_EndL}{ff}.Mask_wCell )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range );
            case 2      % Membrane
                Value = mean(clone_List{idx_EndL}{ff}.CH1.IC(  clone_List{idx_EndL}{ff}.Mask_Memb )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
            case 3      % Old_PL                   
                Value = mean(clone_List{idx_EndL}{ff}.CH1.IC(  clone_List{idx_EndL}{ff}.CH1.Mask_PL_1 )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
            case 4      % New_PL                    
                Value = mean(clone_List{idx_EndL}{ff}.CH1.IC(  clone_List{idx_EndL}{ff}.CH1.Mask_PL_2 )) ; 
                [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
        end
    ...
    while cc < length(cS_EndLine)
        ...
        if ~isempty(idx_TA) && ~isempty(idx_TB)            
            ...
            for ff = 1 : length(clone_List{idx_Anc})
                ...
                switch APP_opt.t3_PlotOpt_C
                    case 1      % Whole Cell
                        Value = mean(clone_List{idx_Anc}{ff}.CH1.IC(  clone_List{idx_Anc}{ff}.Mask_wCell )) ; 
                        [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range );
                    case 2      % Membrane
                        Value = mean(clone_List{idx_Anc}{ff}.CH1.IC(  clone_List{idx_Anc}{ff}.Mask_Memb )) ; 
                        [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
                    case 3      % Old_PL                   
                        Value = mean(clone_List{idx_Anc}{ff}.CH1.IC(  clone_List{idx_Anc}{ff}.CH1.Mask_PL_1 )) ; 
                        [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
                    case 4      % New_PL                    
                        Value = mean(clone_List{idx_Anc}{ff}.CH1.IC(  clone_List{idx_Anc}{ff}.CH1.Mask_PL_2 )) ; 
                        [Value, cRGB] =  Assign_Value_RGB( Value , V_Min, V_Max, RGB_range ); 
                end
...
% --- FINISH layout of plot, RENDER and SAVE the figure ------
h1f.Color = [1 1 1];
ax.TickDir = 'out';                 ax.YTick = [] ;                 
ax.XColor = Color_BorderLines ;     ax.YColor = [1 1 1] ;
ax.LineWidth = LinW ;               ax.FontSize = 15 ;

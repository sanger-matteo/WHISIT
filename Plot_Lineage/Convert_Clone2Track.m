...
%
% Convert_Clone2Track = combine the different clones to form single
%   cellLists that follow the original manual tracks. 
%   This is done if the user decide to plot each independent track as it
%   was tracked irrespective of lineage and cell-to-cell relationship.
%
%   Each clone carries a tag (.ID_ManualTrack) to identify original manual 
%   track. We also know each time point where clones appear. Therefore, it
%   is rather trivial to reassemble the tracks
%
% -------------------------------------------------------------------------
...
for cc = 1 : size(clone_List,2)
    ...
    TrackInfo{2,cc} = clone_List{cc}{1}.frame_birth ;           % when is the first frame of the clone
    TrackInfo{3,cc} = clone_List{cc}{1}.frame_last ;            % when is the last frame of the clone
end
